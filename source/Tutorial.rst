...
All OpenMDAO models are built up from a hierarchy of ``Group`` instances that organize the ``Component``, which is Paraboloid class that we just defined in this example.
...
The output of each ``Component`` feeds into the input of the other, which creates a coupled model that needs to be converged in order for the outputs to be valid. You can see the coupling between the two disciplines show up through the ``y1`` and ``y2`` variables in the following diagram that describes the problem structure:
